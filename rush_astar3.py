...
class Board(object):
    ...
    def update_hor(self, x, y, length, index, step):
        ...
        # If the car moves left, last element should be 0
            ...
            self.config[size -1 - y][x + length] = 0

    def update_ver(self, x, y, length, index, step):
        """
        Determines which values of the board should be changed when a car is moved
        """
        ...
        for i in xrange(length):
            self.config[size -1 -(y + i)][x] = index
        ...
        # If the car moves down, last element should be 0
            ...
            self.config[size -1 - (y + length)][x] = 0
...
class HorCar(Car):
    ...
    def updatePosition(self, step):
        ...
        if (board.isPositionOnBoard(new_xstart, new_ystart) and
            ...
            board.update_hor(new_xstart, new_ystart, self.length, self.index, step)
            return True
...
class VerCar(Car):
    ...
    def updatePosition(self, step):
        ...
        if (board.isPositionOnBoard(new_xstart, new_ystart) and
            ...
            board.update_ver(new_xstart, new_ystart, self.length, self.index, step)
            return True
            ...
            for car in car_objects:
                if car.updatePosition(1):
                    ...
                    if tuple_child not in archive:
                        ...
                        archive[tuple_child] = []
                        stack.push(child, config_depth + 1)
